...
function renderRoute(route) {
  // Always release the camera when leaving (or re-entering) Scan.
  stopScanCamera();
...
function wireScanViewport() {
  ...
  if (startBtn) startBtn.addEventListener('click', startScanCamera);
...
function startScanCamera() {
  ...
  navigator.mediaDevices.getUserMedia(constraints).then(function (stream) {
    ...
    setScanStatus('Camera live. Fill the neon frame, then Capture & Grade.');
  }).catch(function (err) {
